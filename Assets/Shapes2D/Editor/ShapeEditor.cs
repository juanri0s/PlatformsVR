namespace Shapes2D {
    ...
    public class ShapeEditor : Editor {
        ...
        // gets the shape's world corners, not including any children
        Vector3[] GetShapeWorldCorners(Shape shape) {
        ...
        void DrawShapeBorders(Shape shape) {
            Vector3[] corners = GetShapeWorldCorners(shape);
        ...
        bool MouseIsInShapeBounds(Shape shape) {
            Vector3 mouseWorldPos = GetMouseWorldPos();
            Vector3 localPos = shape.transform.InverseTransformPoint(mouseWorldPos);
            Vector2 scale = shape.GetScale() / 2;
            return localPos.x < scale.x && localPos.x > -scale.x && localPos.y < scale.y && localPos.y > -scale.y;
        }
        ...
        void EditPath(Shape shape) {
            ...
            Vector3 mouseWorldPos = GetMouseWorldPos();
            bool isWithinBounds = MouseIsInShapeBounds(shape);
            // drag handle result for getting info from our handles
            CustomHandles.DragHandleResult dhResult;

    ...
    public class Shape : MonoBehaviour, IMaterialModifier {
        ...
        static Vector2[] GetVerticesForPreset(PolygonPreset preset) {
            if (preset == PolygonPreset.Triangle) {
                ...
                return new Vector2[] { new Vector2(0, 0.5f), new Vector2(-0.4f, 0), 
                        new Vector2(-0.15f, 0), new Vector2(-0.15f, -0.5f),
                        new Vector2(0.15f, -0.5f), new Vector2(0.15f, 0),
                        new Vector2(0.4f, 0)
                }; 
            }
        ...
        public class UserProps {
            ...
            private Color _fillColor = new Color(1, 69 / 255f, 0);
            /// <summary>
            ...
            private Color _fillColor2 = new Color(241 / 255f, 196 / 255f, 15 / 255f);
            /// <summary>
            ...
            private float _outlineSize = 0.03f;
            /// <summary>
        ...
        void ApplyShaderPropertiesToMaterial(Material material, bool disableBlending = false) {
            ...
            if (shaderSettings.outlineSize > 0 
                    || shaderSettings.fillType == FillType.MatchOutlineColor)
        ...
        public Vector3[] GetPolygonWorldVertices() {
            ...
            for (int i = 0; i < settings.polyVertices.Length; i++) {
                ...
                v2.x *= scale.x;
                v2.y *= scale.y;
                verts3D[i] = transform.TransformPoint((Vector3) v2);
            }
            return verts3D;
        }
        ...
        public void SetPolygonWorldVertices(Vector3[] verts3D) {
            ...
            for (int i = 0; i < verts3D.Length; i++) {
                Vector2 v2 = transform.InverseTransformPoint(verts3D[i]);
                v2.x /= scale.x;
                v2.y /= scale.y;
                verts[i] = v2;
            }
        ...
        public PathSegment[] GetPathWorldSegments() {
            ...
            for (int i = 0; i < settings.pathSegments.Length; i++) {
                PathSegment segment = settings.pathSegments[i];
                PathSegment worldSegment = new PathSegment(
                        transform.TransformPoint(new Vector3(segment.p0.x * scale.x, segment.p0.y * scale.y, 0)),
                        transform.TransformPoint(new Vector3(segment.p1.x * scale.x, segment.p1.y * scale.y, 0)),
                        transform.TransformPoint(new Vector3(segment.p2.x * scale.x, segment.p2.y * scale.y, 0)));
        ...
        public void SetPathWorldSegments(PathSegment[] worldSegments) {
            ...
            for (int i = 0; i < worldSegments.Length; i++) {
                ...
                Vector2 p0 = transform.InverseTransformPoint(worldSegment.p0);
                p0.x /= scale.x;
                p0.y /= scale.y;
                Vector2 p1 = transform.InverseTransformPoint(worldSegment.p1);
                p1.x /= scale.x;
                p1.y /= scale.y;
                Vector2 p2 = transform.InverseTransformPoint(worldSegment.p2);
                p2.x /= scale.x;
                p2.y /= scale.y;
